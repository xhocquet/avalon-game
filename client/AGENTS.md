...
- Debug console: `Scenes/UI/DebugConsole.tscn` under a game scene root gets bound by `GameNode` and opens on F1 or backtick. It only queues `DebugCommand`/`SetCheatCommand` through `InputCapture`, so everything it does runs in the sim. `Scenes/Playgrounds/CombatPlayground.tscn` carries one; `just playground combat-playground` launches it.
