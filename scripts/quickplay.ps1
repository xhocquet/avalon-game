...
try {
  ...
  $cheatArgs = if ($Godmode) { @("--godmode") } else { @() }

  Write-Host "[quickplay] launching client 1 (faction $Faction1)..."
  $client1 = Start-Process -FilePath $Godot `
    -ArgumentList (@("--path", (Join-Path $repoRoot "client"), "--", "--quickplay", "--faction=$Faction1") + $cheatArgs) -RedirectStandardError "NUL" -PassThru
  ...
  $client2 = Start-Process -FilePath $Godot `
    -ArgumentList (@("--path", (Join-Path $repoRoot "client"), "--", "--quickplay", "--faction=$Faction2") + $cheatArgs) -RedirectStandardError "NUL" -PassThru
