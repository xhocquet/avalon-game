...
| ⬜      | Fix [`TargetAcquisitionSystem`](sim/Systems/TargetAcquisitionSystem.cs)'s O(n²) proximity scan ([`TargetAcquisitionSystem.cs:52`](sim/Systems/TargetAcquisitionSystem.cs:52) nested filter, no spatial broad-phase) — confirmed hotspot via per-system tick timing (up to ~2.7ms/tick at 30v30 by tick 700), same shape as the [`FPNavAvoidance`](vendor/Klotho/com.xpturn.klotho/Runtime/Deterministic/Navigation/FPNavAvoidance.cs) issue. |
