...
public static class GroupFormation {
  ...
  public static void Solve(List<FormationUnit> units, FPVector3 target, MovementRulesAsset rules,
    FPNavMeshQuery query, List<FPVector3> destinations) {
    ...
    var minionTarget = GetMinionTarget(units.Count - heroCount, heroCount, target, forward, rules, query);
    ...
    for (var i = 0; i < units.Count; i++) {
      ...
      destinations.Add(SnapToNavMesh(new FPVector3(heroXz.x, target.y, heroXz.y), query));
    }
  }
  ...
  private static FPVector3 GetMinionTarget(int minionCount, int heroCount, FPVector3 target,
    FPVector2 forward, MovementRulesAsset rules, FPNavMeshQuery query) {
    if (heroCount == 0)
      return SnapToNavMesh(target, query);
    ...
    return SnapToNavMesh(new FPVector3(minionXZ.x, target.y, minionXZ.y), query);
  }
  ...
  private static FPVector3 SnapToNavMesh(FPVector3 slot, FPNavMeshQuery query) {
    if (query == null)
      return slot;
